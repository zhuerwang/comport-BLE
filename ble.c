...
int ble_enquire_name(ble_ctx_t *ble, char *val_buf, int size)
{
    ...
    if(ble->connected == '1')
    {   
        printf("BLE has been connected, Don't send AT to device.\n");
        return -1;
    }

   // printf("atcmd:%s\n", sizeof(atcmd_list[1]));
    if((rv = at_cmd_enquire(ble->comport, atcmd_list[1], val_buf, strlen(atcmd_list[1]), 1500)) < 0)
    {
...
int ble_enquire_addr(ble_ctx_t *ble, char *val_buf,int size)
{
    ...
    if((rv = at_cmd_enquire(ble->comport, atcmd_list[2], val_buf, strlen(atcmd_list[2]), 1500)) < 0)
    {
        printf("Enquire BLE failed.\n");
        return -2;
    }
...
int ble_sleep(ble_ctx_t *ble)
{
    ...
    if((rv = at_cmd_set(ble->comport, atcmd_list[4], strlen(atcmd_list[4]), 1500)) < 0)
    {
        printf("Enquire BLE failed.\n");
        return -2;
    }
...
int ble_wake(ble_ctx_t *ble)
{
    ...
    while(try--)
    {
        comport_write(ble->comport, atcmd_list[5], strlen(atcmd_list[5]));

        memset(ret_buf, 0, sizeof(ret_buf));
        rv = comport_read(ble->comport, ret_buf, sizeof(ret_buf), 1000, block_time);
...
int ble_set_slave_role(ble_ctx_t *ble)
{
    ...
    if((rv = at_cmd_set(ble->comport, atcmd_list[3], strlen(atcmd_list[3]), 1500)) < 0)
    {
        return -2;
    }
...
int ble_check_conn(ble_ctx_t *ble)
{
    ...
    if((rv = comport_read(ble->comport , recv_buf, sizeof(recv_buf), 1000, 1000)) < 0)
    {
        return -1;
    }
...
/* name : ble_recv_data()
 * to do :
 * ble as slave recomportve the data from master and check lost in the same time
 */
int ble_recv_data(ble_ctx_t *ble, char *cjson_str, int size)
{
    ...
    if((rv = comport_read(ble->comport, cjson_str, size, 5000, 100)) < 0)
    {
...
/* name : ble_reply()
 * to do :
 * if ble recomportve data from master will reply the result
 */
int ble_reply(ble_ctx_t *ble)
{
    ...
    if(ble->data_ok == '1')
    {
        if((rv = comport_write(ble->comport, "SLAVE RECIVE OK", strlen("SLAVE RECIVE OK"))) < 0)
        {
    ...
    {
        if((rv = comport_write(ble->comport, SLAVE_ERROR_REPLY, strlen(SLAVE_ERROR_REPLY))) < 0)
        {
            printf("Reply master failed.\n");
            return -2;
        }
